...
namespace Winforms
{
    public partial class MontlySalesFrom : Form
    {
        readonly BillingContext billingContext = new();
        public MontlySalesFrom()
        {
            InitializeComponent();
        ...
        private void FinalCalculation(DateTime dateTime)
        {
            ...
            foreach (MontlyTable montlyTable in billingContext.MontlyTable.Where(x => DbF.DateDiffYear(x.Date, dateTime) == 0 && DbF.DateDiffMonth(x.Date, dateTime) == 0).ToList())
            {
